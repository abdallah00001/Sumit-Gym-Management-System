...
document.getElementById('renewButton').addEventListener('click', async () => {
    ...
    const renewNotes = document.getElementById('renewNotesInput').value
    ...
    try {
        ...
        const paymentObject = {
            ...
            notes: renewNotes, // Additional notes if necessary
        ...
        if (response.ok) {
            ...
            const errorData = await response.text();
            alert(`Failed to renew subscription: ${errorData || 'Unknown error'}`);
        }
    } catch (error) {
...
function updateMemberInfo(member) {
    ...
    const notesInfoSpan = document.getElementById('notesInfo')
    //////////////////////////////////////////////////////
    const pendingSubTypeInfo = document.getElementById('pendingSubscriptionTypeInfo');
    const pendingTrainerInfo = document.getElementById('pendingTrainerInfo');
    const pendingStartDateInfo = document.getElementById('pendingStartDateInfo');
    const pendingExpireDateInfo = document.getElementById('pendingExpireDateInfo');
    const pendingPriceInfo = document.getElementById('pendingPriceInfo');
    const pendingDiscountInfo = document.getElementById('pendingDiscountInfo');
    const pendingFinalPriceInfo = document.getElementById('pendingFinalPriceInfo');
    const pendingNotesInfo = document.getElementById('pendingNotesInfo');

    const pendingSubElementList = [
        pendingSubTypeInfo, pendingTrainerInfo, pendingStartDateInfo, pendingExpireDateInfo,
        pendingPriceInfo, pendingDiscountInfo, pendingFinalPriceInfo, pendingNotesInfo
    ]
    ...
    ///////////////////////////////////
    const pendingSubscription = member.pendingSubscriptionDto;
    if (!pendingSubscription) {
        pendingSubElementList.forEach(element => element.innerText = 'N/A')
    } else {
        pendingSubTypeInfo.innerText = pendingSubscription.subscriptionTypeDto.name || 'N/A';
        pendingTrainerInfo.innerText = pendingSubscription.privateTrainer ? pendingSubscription.privateTrainer.name : 'N/A';
        pendingStartDateInfo.innerText = pendingSubscription.startDate || 'N/A';
        pendingExpireDateInfo.innerText = pendingSubscription.expireDate || 'N/A';
        pendingPriceInfo.innerText = pendingSubscription.subscriptionTypeDto.price || 'N/A';
        pendingDiscountInfo.innerText = pendingSubscription.payments[0].discount || 'N/A';
        pendingFinalPriceInfo.innerText = pendingSubscription.payments[0].finalPrice || 'N/A';
        pendingNotesInfo.innerText = pendingSubscription.notes || 'N/A';
    }
    ...
    if (subscriptionPrice) {
        ...
        document.getElementById('refundAmountInput').value = subscription.payments[0].finalPrice;
    }
}
